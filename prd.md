...
- Web UI 重连时从 EventStore 拉取缺失事件，而不是依赖内存流。
...
- 将所有 RuntimeEvent 写入 EventStore。
...
- 从 Outbox 读取待投递记录。
- 对飞书卡片更新做节流、分片、幂等和重试。
- 对 Web UI 使用 SSE 或 WebSocket 推送；断线后允许按 event seq 补拉。
...
2. EventStore + Outbox：定义事件协议、append-only 存储、投递状态、幂等和重试。
3. Session/Run/Task 状态机：定义状态、并发规则、锁和状态迁移测试。
4. RuntimeSupervisor 契约：实现进程生命周期、stdout/stderr 解码、取消、超时、退出码和清理。
5. 三个 MVP 适配器：跑通 Codex CLI、Claude Code、Trae CLI 的成功接入路径。
6. Agent 切换与交接：支持新建会话选择 Agent、默认 Agent、handoff 新会话。
7. 上下文预算与 ContextPack：支持 token 估算、自动压缩、恢复和原始历史追溯。
8. Persistence：保存会话、run、task、事件、outbox、ContextPack、handoff 和 audit log。
9. 精致 Web UI：完成多会话工作台、流式输出、上下文状态、运行状态和基础动效。
10. 飞书 adapter：接收飞书消息并通过 Outbox 返回流式卡片。
11. Scheduler：支持一次性任务和 cron 任务。
12. 记忆归档：每日原始记录和摘要。
13. 扩展 Telegram、QQ、微信和更多 Agent runtime。
