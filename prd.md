...
6. Persistence：保存会话、消息、事件、运行记录和 handoff 关系。
7. 精致 Web UI：完成多会话工作台、流式输出、运行状态和基础动效。
8. 飞书 adapter：接收飞书消息并返回流式卡片。
9. Scheduler：支持一次性任务和 cron 任务。
10. 记忆归档：每日原始记录和摘要。
11. 扩展 Telegram、QQ、微信和更多 Agent runtime。
