...
UI 方向采用 prototype `A — Command Deck`。这是 MVP 的默认工作台形态：左侧会话/任务导航，中间主对话与流式输出，右侧运行状态、ContextPack、Outbox、handoff 操作和关键事件。其他 prototype 方向仅作为局部参考，不作为第一版主布局。

正式实现必须尽量保留 `A — Command Deck` 的三栏视觉层级和核心交互模型。除非存在明确的实现约束或可验证的可用性问题，否则不应偏离该原型的主布局。

体验要求：

- 首屏就是可操作工作台：左侧会话/任务导航，中间对话流，右侧运行状态、上下文状态、Outbox、工具调用和关键事件。
- 会话列表必须清楚显示 Agent 类型、运行状态、工作目录、是否由 handoff 创建。
- 右侧状态栏必须优先展示 RuntimeSupervisor、ContextPack、Outbox 和 handoff 操作，不做纯装饰面板。
- 支持浅色/深色主题，默认使用系统主题。
