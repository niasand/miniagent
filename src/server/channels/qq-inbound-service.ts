...
export type QQInboundMessageInput = {
  messageId: string;
  chatId: string;
  userId?: string | null;
  text: string;
...
export type QQInboundOptions = {
  workspacePolicy?: WorkspacePolicy;
};

const AGENTS: AgentType[] = ["codex", "claude", "trae"];

export class QQInboundService {
  ...
  constructor(private readonly db: SqliteDatabase, events = new EventStore(db), options: QQInboundOptions = {}) {
  ...
  receiveMessage(input: QQInboundMessageInput): QQInboundResult {
    ...
    if (command?.name === "agent" && command.args[0] === "new") {
      ...
      const session = this.sessions.createSession({
        title: `QQ ${displayAgent(agentType)}`,
        agentType,
        workspacePath,
        channelType: "qq",
  ...
  private resolveSession(input: QQInboundMessageInput): SessionRecord {
    if (input.sessionId) {
  ...
  private createMessageTask(input: QQInboundMessageInput, session: SessionRecord, text: string): TaskRecord {
    const dedupeKey = `qq:${input.messageId}`;
    try {
  ...
  private auditRemoteCommand(input: QQInboundMessageInput, action: string, payload: JsonObject): void {
    this.auditLogs.insert({
      actorType: "qq_user",
      actorRef: input.userId ?? null,
      action,
      resourceType: "qq_message",
  ...
  private requireAllowedWorkspace(input: QQInboundMessageInput, workspacePath: string, action: string): string {
    try {
  ...
  private resolveDefaultAgent(input: QQInboundMessageInput, workspacePath: string): AgentType {
    return (
