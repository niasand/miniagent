...
export type DiscordInboundMessageInput = {
  messageId: string;
  chatId: string;
  userId?: string | null;
  text: string;
...
export type DiscordInboundOptions = {
...
export class DiscordInboundService {
  ...
  constructor(private readonly db: SqliteDatabase, events = new EventStore(db), options: DiscordInboundOptions = {}) {
  ...
  receiveMessage(input: DiscordInboundMessageInput): DiscordInboundResult {
    ...
    if (command?.name === "agent" && command.args[0] === "new") {
      ...
      const session = this.sessions.createSession({
        title: `Discord ${displayAgent(agentType)}`,
        agentType,
        workspacePath,
        channelType: "discord",
  ...
  private resolveSession(input: DiscordInboundMessageInput): SessionRecord {
    if (input.sessionId) {
  ...
  private createMessageTask(input: DiscordInboundMessageInput, session: SessionRecord, text: string): TaskRecord {
    const dedupeKey = `discord:${input.messageId}`;
    try {
  ...
  private auditRemoteCommand(input: DiscordInboundMessageInput, action: string, payload: JsonObject): void {
    this.auditLogs.insert({
      actorType: "discord_user",
      actorRef: input.userId ?? null,
      action,
      resourceType: "discord_message",
  ...
  private requireAllowedWorkspace(input: DiscordInboundMessageInput, workspacePath: string, action: string): string {
    try {
  ...
  private resolveDefaultAgent(input: DiscordInboundMessageInput, workspacePath: string): AgentType {
    return (
