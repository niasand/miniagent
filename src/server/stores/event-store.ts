...
export class EventStore {
  ...
  listAfterGlobalSeq(options: { sessionId?: string; afterGlobalSeq: number; limit?: number }): StoredEvent[] {
    ...
    const sql = options.sessionId
      ? "SELECT * FROM events WHERE session_id = ? AND global_seq > ? ORDER BY global_seq ASC LIMIT ?"
      : "SELECT * FROM events WHERE global_seq > ? ORDER BY global_seq ASC LIMIT ?";

    const rows = options.sessionId
      ? this.db.prepare(sql).all(options.sessionId, options.afterGlobalSeq, limit)
      : this.db.prepare(sql).all(options.afterGlobalSeq, limit);
  ...
  private insertEvent(input: AppendEventInput): StoredEvent {
    ...
    const row = this.db.prepare(
      `INSERT INTO events (id, session_id, run_id, task_id, run_seq, type, payload_json, schema_version, causation_id, correlation_id, created_at)
       VALUES (@id, @sessionId, @runId, @taskId, @runSeq, @type, @payloadJson, @schemaVersion, @causationId, @correlationId, @createdAt)
       RETURNING *`
    ).get({
