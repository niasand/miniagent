...
export class RuntimeSupervisor {
  ...
  startTask(input: StartRuntimeTaskInput): StartedRuntimeRun {
    ...
    const adapter = this.adapterRegistry.get(agentType);
    ...
    try {
      ...
      process.onOutput((chunk) => this.handleOutput(run.id, chunk));
      process.onExit((exit) => this.handleExit(run.id, exit));
  ...
  sendInput(runId: string, input: RuntimeInput): void {
    ...
    activeRun.process.write(activeRun.adapter.encodeInput(input));
  }

  stop(runId: string): void {
    this.requireActiveRun(runId).process.stop("SIGTERM");
  }
  ...
  private handleOutput(runId: string, chunk: RuntimeOutputChunk): void {
    const activeRun = this.activeRuns.get(runId);
    if (!activeRun) {
      return;
    }

    const drafts = activeRun.adapter.decodeOutput(chunk);
  ...
  private handleExit(runId: string, exit: RuntimeProcessExit): void {
    ...
    const classification = classifyExit(activeRun.adapter, exit);
    this.options.sessionStore.finishRun({
      runId,
...
function classifyExit(adapter: AgentRuntimeAdapter, exit: RuntimeProcessExit): RuntimeErrorClassification {
